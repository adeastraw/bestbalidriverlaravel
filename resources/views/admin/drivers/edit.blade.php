...
@section('header_title', 'Edit Driver Profile')

@section('content')
    <div class="max-w-3xl space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="font-display text-2xl font-bold text-forest-900">Edit Driver: {{ $driver->name }}</h1>
            <a href="{{ route('admin.drivers.index') }}" class="text-xs font-semibold text-sand-600 hover:underline">
                ← Back to Drivers List
            </a>
        </div>

        <form action="{{ route('admin.drivers.update', $driver->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-2xl border border-sand-200 shadow-sm space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="slug" class="block text-xs font-bold text-sand-700 uppercase tracking-wider mb-1">URL Slug</label>
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $driver->slug) }}" 
                           class="w-full rounded-xl border-sand-300 text-sm focus:border-emerald-600 focus:ring-emerald-600">
                </div>

                <div>
                    <label for="experience" class="block text-xs font-bold text-sand-700 uppercase tracking-wider mb-1">Experience</label>
                    <input type="text" name="experience" id="experience" value="{{ old('experience', $driver->experience) }}" 
                           class="w-full rounded-xl border-sand-300 text-sm focus:border-emerald-600 focus:ring-emerald-600">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="languages" id="languages" value="{{ old('languages', $driver->languages) }}" 
                           class="w-full rounded-xl border-sand-300 text-sm focus:border-emerald-600 focus:ring-emerald-600">
                </div>

                <div>
                    <label for="service_area" class="block text-xs font-bold text-sand-700 uppercase tracking-wider mb-1">Service Area</label>
                    <input type="text" name="service_area" id="service_area" value="{{ old('service_area', $driver->service_area) }}" 
                           class="w-full rounded-xl border-sand-300 text-sm focus:border-emerald-600 focus:ring-emerald-600">
                </div>
            </div>

            <div class="flex items-center gap-4">
                <img src="{{ $driver->photo_url }}" alt="{{ $driver->name }}" class="w-16 h-16 rounded-full object-cover border border-sand-200">
                <div class="flex-grow">
                    <label for="photo" class="block text-xs font-bold text-sand-700 uppercase tracking-wider mb-1">Change Profile Photo</label>
            ...
            <div>
                ...
                <input type="text" name="short_bio" id="short_bio" value="{{ old('short_bio', $driver->short_bio) }}" 
                       class="w-full rounded-xl border-sand-300 text-sm focus:border-emerald-600 focus:ring-emerald-600">
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="5" 
                          class="w-full rounded-xl border-sand-300 text-sm focus:border-emerald-600 focus:ring-emerald-600">{{ old('description', $driver->description) }}</textarea>
            </div>

            <div class="flex items-center gap-6 pt-2">
                <label class="flex items-center gap-2 cursor-pointer text-sm text-sand-800">
                    <input type="checkbox" name="status" value="1" {{ old('status', $driver->status) ? 'checked' : '' }}
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $driver->sort_order) }}" class="w-20 rounded-xl border-sand-300 text-sm">
                </div>
            </div>
