                    ...
                    @forelse($drivers as $driver)
                        <tr class="hover:bg-cream-50 transition-colors">
                            <td class="p-4 flex items-center gap-3">
                                <img src="{{ $driver->photo_url }}" alt="{{ $driver->name }}" class="w-10 h-10 rounded-full object-cover">
                                <div>
                                    <span class="font-bold text-forest-900 block">{{ $driver->name }}</span>
                                    <span class="text-xs text-sand-500">{{ $driver->phone ?: 'No phone' }}</span>
                                </div>
                            </td>
                            <td class="p-4 text-sand-700">{{ $driver->experience ?: '—' }}</td>
                            <td class="p-4 text-sand-700 text-xs">{{ $driver->languages ?: '—' }}</td>
                            ...
                            <td class="p-4 text-right space-x-2">
                                <a href="{{ route('admin.drivers.edit', $driver->id) }}" class="text-xs font-semibold text-emerald-700 hover:underline">
                                    Edit
                                </a>
